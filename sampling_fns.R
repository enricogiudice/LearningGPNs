set.searchspace <- function(data, dual, method, par = 1) {
  ...
  list(score = score, scoretable = searchspace$scoretable, 
       DAG = searchspace$DAG, maxorder = searchspace$maxorder)
}
...
GP.partition.mcmc <- function(data, searchspace, alpha = 0.05, 
  ...
  if(order) {
    ...
    fit <- partitionMCMC(myScore, alpha = alpha, startDAG = searchspace$DAG,
                        scoretable = searchspace$scoretable, iterations = iterations, stepsave = 4)
  }
